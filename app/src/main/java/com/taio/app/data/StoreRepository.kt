...
class AppRepository(private val store: StoreRepository = StoreRepository()) {
    ...
    // ---------- Shipping rates (per governorate, نفس مفتاح الموقع settings:shipping) ----------
    suspend fun getShippingRates(): Map<String, Double> {
        val raw = store.get("settings:shipping") ?: return emptyMap()
        val o = org.json.JSONObject(raw)
        val map = mutableMapOf<String, Double>()
        o.keys().forEach { k -> map[k] = o.optDouble(k, DEFAULT_SHIPPING) }
        return map
    }

    suspend fun saveShippingRates(rates: Map<String, Double>) {
        val o = org.json.JSONObject()
        rates.forEach { (k, v) -> o.put(k, v) }
        store.set("settings:shipping", o.toString())
    }
}
